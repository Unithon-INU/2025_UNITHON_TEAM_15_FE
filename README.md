...
### 인앱 언어가 시스템 언어로 되돌아가는 버그 전면 차단

한국어/영어 인앱 언어 전환을 지원했는데, 사용자가 언어를 바꾸지 않아도 앱 언어가 갑자기 시스템 언어로 바뀌었다 돌아오는 버그가 여러 화면에서 발생했습니다. 추적 결과 WebView 등 외부 컴포넌트가 전역 Locale을 덮어쓰는 것이 원인이었습니다. 발생 지점을 하나씩 막는 대신, 화면 갱신·복귀 시마다 저장된 언어 설정으로 복원하는 방어 로직을 두어 원인과 무관하게 전면 차단했습니다.
